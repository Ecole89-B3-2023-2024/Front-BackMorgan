...
router.post("/add", auth, async (req, res) => {
    try {
        req.body.loginAuthor = req.payload.email;
        ...
        return res.status(500).json({test: req.payload, message: error.message});
    }
})

router.get("/all", async (req, res) => {
    try {
        const videoList = await Video.findAll({ order: [['createdAt', 'DESC']] });
        return res
            .status(200)
